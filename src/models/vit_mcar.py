...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, device):
        ...
        gf = feats_pooling(ga, method='avg', sh=int(h/16), sw=int(w/16))                
        gf = self.convclass(gf)             #bxcx1x1
        # gs = torch.sigmoid(gf)              #bxcx1x1          
        gs = gf.view(gf.size(0), -1)        #bxc
        # cls1 = self.part_head(part_encoded[:, 0])

        camscore = self.convclass(ga.detach())
        camscore = get_cams(attn_weights1, camscore, device)
        ...
        for i in range(b):
            ...
            for j in range(topN):
                ...
                x1, x2 = obj_loc(xs, threshold=0.5)
                y1, y2 = obj_loc(ys, threshold=0.5)
                linputs[i:i+1, j ] = F.interpolate(x[i:i+1, :, y1:y2, x1:x2], size=(h, w), mode='bilinear', align_corners=True)
        ...
        # ls = torch.sigmoid(lf)
        ls = F.max_pool2d(lf.reshape(b, topN, self.num_classes, 1).permute(0,3,1,2), (topN, 1))
        ls = ls.view(ls.size(0), -1)
        # cls2 = self.part_head(part_encoded2[:, 0])
        # cls2 = cls2.reshape(b, topN, self.num_classes).unsqueeze(1)
        # cls2 = F.max_pool2d(cls2, (topN,1)).squeeze(1).squeeze(1)
